...
Instead of applying one global LoRA strength to the whole model, this node reads the LoRA weight keys, maps them into a hierarchical tree such as `UNET / DOWN / BLOCK\_x / ...` and `TEXT\_ENCODER / ...`, and lets you adjust the strength of different branches independently.
...
Clone this repository into your ComfyUI `custom\_nodes` directory:
...
1. Add **Hierarchical LoRA Loader** to your workflow.
2. Connect `MODEL` and `CLIP` inputs.
3. Select a LoRA from the `lora\_name` dropdown.
4. Set global strengths:

   * `strength\_model`: global multiplier for UNet/model LoRA patches.
   * `strength\_clip`: global multiplier for CLIP/text-encoder LoRA patches.
5. Click **Open Block Editor**.
6. Adjust strengths for different branches.
7. Click **Apply**.
...
```text
final\_strength = global\_strength × effective\_block\_strength × curve\_multiplier
```

Where:

```text
effective\_block\_strength = parent\_strength\_1 × parent\_strength\_2 × ... × local\_block\_strength
```
...
UNET = 0.8
UNET/DOWN = 0.5
UNET/DOWN/BLOCK\_0 = 1.2
```

Then the effective strength for `UNET/DOWN/BLOCK\_0` is:

```text
0.8 × 0.5 × 1.2 = 0.48
```

If `strength\_model = 1.0`, the final model LoRA strength for that block is `0.48`.
...
The editor writes its result into the hidden `tree\_config\_json` field.
...
This means the LoRA effect for `UNET/DOWN/BLOCK\_0` starts at `0`, rises to `1.0` around 40% sampling progress, and ends at `0.5`.
...
Select a LoRA in the `lora\_name` dropdown before opening the editor.
...
Try increasing `strength\_model` first. The final strength is the product of the global strength and the hierarchical effective strength.
...
```text
\_\_init\_\_.py                         ComfyUI node registration
nodes.py                            Node definition and HTTP tree endpoint
lora\_parser.py                      LoRA key parsing and file hash cache
strength\_tree.py                    Key-to-tree-path mapping and strength inheritance
weight\_apply.py                     Static and dynamic LoRA patch application
web/js/tree\_lora\_loader.js          Frontend block editor
```

The frontend requests the parsed LoRA tree from:

```text
/hierarchical\_lora\_loader/tree?lora\_name=...
```
